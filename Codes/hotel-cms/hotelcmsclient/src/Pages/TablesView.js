...
function TablesView() {
  ...
  const roomsGet = () =>{
    fetch("http://localhost:5000/diningtables", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  const diningDelete = (Id) =>{
    fetch("http://localhost:5000/diningtable", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div class="main">
      <div class="menu">
        <p class="menu1">
          CONTENT MANAGEMENT SYSTEM || <b>FINEST TOKYO HOTEL©</b> || DINING TABLES
        </p>
          ...
          <table class="styled-table">
            ...
            <tbody>
              ...
              <tr class="row" key={i}>
                ...
                <td>{datum.TableCapacity}</td>
                <td>{datum.DiningStatus}</td>
                <td><button class="button" onClick={() => diningDelete(datum.Id)}>DELETE</button></td>
              </tr>
              ))}
            </tbody>
          </table>
          <p class="addbuttonroom"><Link to="/AddDiningTableForm"  class="button" style={{ textDecoration: 'none' }}><b>ADD TABLE</b></Link></p>
        </div>
      </div>
    </div>
  );
