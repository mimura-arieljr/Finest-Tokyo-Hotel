...
function DiningBookingsView() {
  ...
  const roomsGet = () =>{
    fetch("http://localhost:5000/diningbookings", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  const diningBookingDelete = (Id) =>{
    fetch("http://localhost:5000/diningbooking", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div class="main">
      <div class="menu">
        <p class="menu1">
          CONTENT MANAGEMENT SYSTEM || <b>FINEST TOKYO HOTEL©</b> || DINING BOOKINGS
        </p>
          ...
          <thead>
            ...
            {data.filter((datum) => {
              ...
              <tr class="row" key={i}>
                ...
                <td>{datum.DiningId}</td>
                <td>{datum.TableId}</td>
                <td>{datum.BookingDate}</td>
                <td>{datum.CheckInDate}</td>
                <td>{datum.PaymentType}</td>
                <td>{datum.BookingStatus}</td>
                <td><button class="button" onClick={() => diningBookingDelete(datum.Id)}>DELETE</button></td>
              </tr>
              ))}
            </tbody>
          </table>
...
export default DiningBookingsView;
